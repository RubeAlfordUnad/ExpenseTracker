...
struct BackupRestoreView: View {
            ...
            HStack(alignment: .top, spacing: 14) {
                VStack(alignment: .leading, spacing: 6) {
                    ...
                    Text(
                        ...
                        ? "Exporta un archivo JSON con gastos, ingresos, deudas, pagos fijos, presupuesto, preferencias y foto de perfil."
                        : "Export a JSON file with expenses, incomes, debts, recurring payments, budget, preferences, and profile image."
                    )
                    .font(.subheadline)
    ...
    private var restoreNotesCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            noteRow(
                ...
                textES: "La restauración reemplaza gastos, ingresos, deudas, pagos fijos, presupuesto, preferencias y foto de perfil del usuario actual.",
                textEN: "Restoring replaces the current user's expenses, incomes, debts, recurring payments, budget, preferences, and profile image."
            )
    ...
    private func importConfirmationMessage(for snapshot: AppBackupSnapshot) -> String {
        ...
        if settings.language == .spanish {
            return "Archivo de \(snapshot.sourceUser) · \(dateText)\n\nSe importarán \(summary.expensesCount) gastos, \(summary.incomesCount) ingresos, \(summary.debtsCount) deudas y \(summary.recurringPaymentsCount) pagos fijos.\n\nEsto reemplazará los datos actuales del usuario \"\(currentUserLabel)\"."
        } else {
            return "File from \(snapshot.sourceUser) · \(dateText)\n\nThis will import \(summary.expensesCount) expenses, \(summary.incomesCount) incomes, \(summary.debtsCount) debts, and \(summary.recurringPaymentsCount) recurring payments.\n\nThis will replace the current data for user \"\(currentUserLabel)\"."
        }
    }
}
